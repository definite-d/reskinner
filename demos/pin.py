...
def main():
    ...
    menudef = [
        ["File", ["Exit"]],
        ["Demo", ["Toggle Shrink", "Toggle NoShrink", "Toggle Regular"]],
    ]
    ...
    layout = [
        ...
        [
            ...
            sg.Text(
                "When shrink=True, single pixel of old theme remains after reskinning",
                text_color="red",
            ),
        ],
        [
            ...
            sg.pin(
                sg.Button("Shrink=False Button", key="-NOSHRINK_BTN-"), shrink=False
            ),
        ...
        [
            ...
            sg.pin(
                sg.Input("Shrink=False Input", key="-NOSHRINK_INPUT-", size=20),
                shrink=False,
            ),
        ...
        [
            sg.StatusBar(
                "Change theme to see the single pixel issue with shrink=True elements",
                key="-STATUS-",
            ),
        ],
    ]
    ...
    for element in window.element_list():
        ...
        if hasattr(element, "Widget") and hasattr(element.Widget, "winfo_children"):
            print(
                f"    Children: {[type(child).__name__ for child in element.Widget.winfo_children()]}"
            )
    ...
    while True:
        ...
        if event == "-THEME-":
            ...
            window["-STATUS-"].update(
                f"Reskinned to {new_theme} - Check for single pixel artifacts!"
            )
